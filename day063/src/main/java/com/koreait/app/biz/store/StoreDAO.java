...
public class StoreDAO {
	...
	final String INSERT = "INSERT INTO BB_STORE(STORE_NAME, STORE_ADDRESS, STORE_ADDRESS_DETAIL, STORE_CONTACT) VALUES (?, ?, ?, ?)";
	
	//특정가게 상세정보 조회
	//받은 데이터 : 가게 고유번호(PK)
	//조회 데이터 : 가게 고유번호(PK), 가게 상호명, 기본&상세주소, 가게 전화번호, 가게폐점여부
	final String INFO_STORE_SELECTONE = "SELECT STORE_NUM, STORE_NAME, STORE_ADDRESS, STORE_ADDRESS_DETAIL, STORE_CONTACT, STORE_CLOSED FROM BB_STORE WHERE STORE_NUM = ?";
	...
	//폐점 안 한 가게 수 조회
	final String STORE_CNT_SELECTONE = "SELECT COUNT(STORE_NUM) AS STORE_COUNT FROM BB_STORE WHERE STORE_CLOSED = ?";
	...
	final String NAME_LIKE = "AND STORE_NAME LIKE CONCAT('%', ?, '%')";
	...
	final String SELECTALL_STORE_DATAS = "SELECT STORE_NUM, STORE_NAME, STORE_ADDRESS, STORE_ADDRESS_DETAIL, STORE_CONTACT, STORE_CLOSED FROM BB_STORE WHERE STORE_NUM IN (";

	// [2] input값으로 들어온 조건 기반 가게 고유번호PK들: 1,3,4,5,6...

	// [3] 가게 고유번호(PK) 역순정렬 + 페이지네이션 요소
	final String SELECTALL_ELEMENT_DESC_PAGENATION = ") ORDER BY STORE_NUM DESC LIMIT ?, ?";
...
// FILTER_NUM_CNT_SELECTALL : 필터링된 가게 고유번호 반환 메서드---------------------------------------------------------------------------ㄴ
			...
			try {
				...
				= "SELECT DISTINCT STORE_NUM FROM BB_VIEW_SEARCHSTOREDATA_JOIN";
...
// 가게 기본 정보 : 가게 고유번호, 상호명, 기본주소, 상세주소, 전화번호, 폐점여부
			...
			try {
				...
				pstmt.setInt(1, storeDTO.getStoreStartPage()-1); 	// 페이지네이션 시작 번호
				//DBMS를 변경하며 between -> limit로 변경되고 between은 시작번호를 포함하지만 limit는 그러지 않으므로 -1 연산추가
