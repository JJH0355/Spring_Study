...
import comm.koreait.app.view.common.ErrorUtils;
 ...
 */
@Controller("viewProductMDAction")
public class ViewProductMDAction{
    /**
    ...
    public ModelAndView viewProductMDA(HttpServletRequest request, HttpServletResponse response, ModelAndView mav, ProductDTO productDTO, BoardDTO boardDTO, ImageDTO imageDTO, @CookieValue(name = "viewedProducts") Cookie cookie) {
        ...
        if (cookies != null) {
            for (Cookie cookieData : cookies) {
                if (cookieData.getName().equals("viewedProducts")) {
                   // URL 디코딩
                    try {
                  viewedProducts = URLDecoder.decode(cookieData.getValue(), "UTF-8");
